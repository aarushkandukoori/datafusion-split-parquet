...
use datafusion::physical_optimizer::pruning::{PruningPredicate, build_pruning_predicate};
...
use bytes::Bytes;
use datafusion::arrow::array::{ArrayRef, Datum, Int32Array, RecordBatch, StringArray};
...
async fn main() -> Result<()> {
    ...
    let provider = Arc::new(ZippedTableProvider::try_new(
    ...
    let df = ctx.sql("SELECT b FROM zip_table WHERE a = 100").await?;
    df.show().await?;
    Ok(())
}
...
impl ExecutionPlan for ZipExec {
    ...
    fn execute(
        ...
        Ok(Box::pin(ZipStream {
            schema: Arc::clone(&self.schema),
            left_input: left_stream,
            right_input: right_stream,
        }))
    }
}
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let poll = match (
            ...
            (Poll::Ready(Some(Ok(left_batch))), Poll::Ready(Some(Ok(right_batch)))) => {
                println!(
                    "both batches are good, zipping: lhs = {} ({}), rhs = {} ({})",
                    left_batch
                        .schema()
                        .flattened_fields()
                        .iter()
            ...
            (Poll::Ready(Some(Ok(left_batch))), Poll::Ready(None)) => {
                println!("Right side is none");
                Poll::Ready(None)
            }
            (Poll::Ready(None), Poll::Ready(Some(Ok(right_batch)))) => {
                println!("Left side is none");
                Poll::Ready(None)
            }
            (Poll::Ready(Some(Err(e))), _) => {
                println!("error on left side");
                Poll::Ready(Some(Err(e)))
            }
            (_, Poll::Ready(Some(Err(e)))) => {
                println!("error on right side");
                Poll::Ready(Some(Err(e)))
            }
            (Poll::Ready(None), Poll::Ready(None)) => Poll::Ready(None),
            (l_other, r_other) => Poll::Pending,
        };
        poll
    }
